 ...
 */
template <typename Iterator, typename IndexList>
void GridSample(Iterator first, Iterator last, double resolution,
    ...
    Octree<bool> octree(size1, size2, size3); //构建八叉树
    ...
    for (int s : random_seq) {
        ...
        x = Clamp(x, 0, size1 - 1); //限定x的区间在0和size-1之间
        y = Clamp(y, 0, size2 - 1);
        z = Clamp(z, 0, size3 - 1);
