...
public:
    ...
    void FindKNearestNeighbors(const Point &p, int k,
        ...
        for(auto i : results.indices)
        {
            neighbors->push_back(i);
        }
        Array<int>().swap(results.indices);
    ...
    void FindKNearestNeighbors(const Point &p, int k,
        ...
        //neighbors->resize(indices.size()); //resize会经常runtime error
        neighbors->clear();
        for (int i = 0; i < indices.size(); ++i) {
            neighbors->emplace_back( points_[indices[i]] );
        }
    }
...
private:
    ...
    void PlaneSplit(int left, int right, int cut_dimension, const T &cut_value,
        ...
        for (;;) {
            while (l <= r &&
                    points_[indices_[l]][cut_dimension] < cut_value) {
                ++l;
            }
            while (r && l <= r &&
                    points_[indices_[r]][cut_dimension] >= cut_value) {
                --r;
            }
            if (l > r || !r) {
                break;
